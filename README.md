## ethminer-genoil-chunked

What is ethminer-genoil-chunked?

ethminer-genoil-chunked is a fork of [Genoil's miner](https://github.com/Genoil/cpp-ethereum) that incorporates [Equinox's chunked mining pull request](https://github.com/ethereum/libethereum/pull/203) that was never accepted into the official ethminer, nor included by genoil.

With this fork, ethereum mining works on R9 270 GPUs that otherwise always fail due to DAG size regardless what env vars are used.  7970 cards are normally OK with DAG size, but sometimes freak out until next reboot and they also work with this "chunky" fork when in the freaked-out mode.

This fork is based on Genoil's code from a few months back and there are no plans to update it to the latest code as the chunked patch does not apply cleanly -- and anyway it does everything I need it to do.  Namely, it works with my GPUs.
